...
export default function Slot() {
  ...
  useEffect(async () => {
    ...
    let CHAIN_ID = process.env.NEXT_PUBLIC_ENV_CHAINID;
    await window.keplr.experimentalSuggestChain({
      chainId: CHAIN_ID,
      chainName: "test net",
      rpc: process.env.NEXT_PUBLIC_ENV_RPC,
      rest: process.env.NEXT_PUBLIC_ENV_REST,
      bip44: {
        coinType: 529,
      },
      bech32Config: {
    ...
    const grpcWebUrl = process.env.NEXT_PUBLIC_ENV_GRPC;
    ...
    const win_table = await quary_win_table(pid, codeHash, secretjs);
